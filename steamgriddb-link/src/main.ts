...
async function getSgdbLink(appId: string): Promise<string | null> {
  const apiKey = GM_getValue<string>("sgdb_api_key");
  if (!apiKey) {
...
function initApiKeyMenu() {
  GM_registerMenuCommand("Set SteamGridDB API Key", () => {
    const current = GM_getValue<string>("sgdb_api_key", "");
    const key = prompt("Enter your SteamGridDB API Key:", current);
    if (key !== null) {
      GM_setValue("sgdb_api_key", key.trim());
...
async function main() {
  ...
  const config = configs.find((c) =>
    window.location.hostname.includes(c.hostname),
  );
  if (config) {
    initApiKeyMenu();
